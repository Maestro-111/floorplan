...
from room_floor_area_ocr import run
...
def create_metedata_folders():
    ...
    after wee got 3 outputs for each image in result folder, we do thee following:

    1) create 2 folders, coords and imgs. Put them into folder called test_boxes_from_craft
    2) Next, populate coords with the coords of each ROI (text regions) and populate imgs with the image which containes ROIS
    3) Creare a zip collection from coords and imgs. then return it
    ...
    for folder in os.listdir('test_boxes_from_craft'):
        path = os.path.join('test_boxes_from_craft',folder)
...
parser.add_argument('--area_floor_rooms', default=False, help='do area,floor number and rooms analysis')
parser.add_argument('--area_floor_rooms_save_path', default=r'C:\floorplan\unit_testing\craft_ocr_output.xlsx',type=str, help="unit test save locatinon")
parser.add_argument('--unit_number_save_loc',type=str, help="save location for unit numbers")
parser.add_argument('--key_plates', default=False, help='do key plates analysis')
parser.add_argument('--unit_testing_save_path', default=r'C:\floorplan\Seneca_Phase3\unit_testing',type=str, help='unit test save locatinon')
